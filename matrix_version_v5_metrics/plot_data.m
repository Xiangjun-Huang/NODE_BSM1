...
function plot_data(tyRow, rName, t, yData, opt)
    arguments
        tyRow (:,2) uint64;
        rName (:,1) string;
    ...
    for i = 1:opt.nReactors
        ...
        for k = 1:opt.nComponents
            ...
            hold on
            for j = 1:nYData
                plot(t(tyRow(i,1):tyRow(i,2),1), yData{j}(tyRow(i,1):tyRow(i,2),k), LineWidth=1)
            end
            hold off
            
            strSubTitle = opt.yName(k);
            if nYData >= 2
                yIKTrue = yData{1}(tyRow(i,1):tyRow(i,2),k);
                yIKPred = yData{2}(tyRow(i,1):tyRow(i,2),k);
                if opt.showRMSE
        ...
        lgd = legend(opt.yLabels,'Orientation','vertical');
        lgd.Position(1) = 0.82;
        lgd.Position(2) = 0.08;
